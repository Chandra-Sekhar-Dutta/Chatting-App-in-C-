...
//This tells the compiler to link the Winsock library (ws2_32.lib) automatically.
//Required for Windows network programming.
...
void sendMessage(SOCKET clientSocket) {
	...
	while (true) {
		...
		//send() returns the number of bytes which can be signed interger also
...
void RecieveMessage(SOCKET clientSocket) {
	...
	while (true) {
		...
		//recv() returns the number of bytes which can be signed interger also
